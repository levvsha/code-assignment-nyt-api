import newsReducer, { getInitialState } from '../news';
...
describe('news reducer', () => {
  ...
  it('initial state is correct', () => {
    expect(newsReducer(undefined, {})).toMatchSnapshot();
  });

  it('+++ works for START_REQUEST action type', () => {
    afterState.isLoading = true;

    expect(newsReducer(beforeState, { type: 'news/START_REQUEST' })).toEqual(afterState);
  });

  it('+++ works for FINISH_REQUEST action type', () => {
    beforeState.isLoading = true;
    afterState.isLoading = false;

    expect(newsReducer(beforeState, { type: 'news/FINISH_REQUEST' })).toEqual(afterState);
  });

  it('+++ works for SET_LIST action type', () => {
    afterState.pageOffset = 1;
    afterState.newsList = expectedNewsList;
    ...
    expect(newsReducer(beforeState, action)).toEqual(afterState);
  });

  it('+++ works for ADD_NEW_ITEMS action type', () => {
    beforeState.newsList = expectedNewsList;
    afterState.pageOffset = afterState.pageOffset + 1;
    afterState.newsList = expectedNewsList.concat(expectedNewsList);
  ...
  it('+++ works for UPDATE_SEARCH_QUERY action type', () => {
    afterState.searchQuery = 'foo';
  ...
  it('+++ works for SET_ERROR_MESSAGE action type', () => {
    afterState.errorMessage = 'error happen';
  ...
  it('+++ works for SET_EXPANDED_NEWS_ID action type', () => {
    afterState.expandedNewsId = 123;
